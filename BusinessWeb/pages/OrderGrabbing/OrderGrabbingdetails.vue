	...
	<view>
		...
		<view class="top">
			<view style="display: flex;justify-content: space-between;width: 100%;">
				<view class="">
					收货人：{{order.UserAddr.name}}
				</view>
				<view class="">{{order.UserAddr.mobile}}</view>
			</view>
			<view class="address">
				收货地址：{{order.UserAddr.province==order.UserAddr.city?order.UserAddr.province+order.UserAddr.area+order.UserAddr.addr:order.UserAddr.province+order.UserAddr.city+order.UserAddr.area+order.UserAddr.addr}}
			</view>
		...
		<view class="item">
			<view class="title">
				<view class="ShopName">{{order.shop.shop_name}}</view>
			</view>
			<view class="shop">
					<view class="row">
						{{order.content}}
					...
					<view class="row">
						<view>价格区间(元)</view>
						<view>{{order.min_price}}-{{order.max_price}}</view>
					</view>
			</view>
			...
			<!-- <view class="row">
				...
				<view class="newPrice">¥{{order.logistic}}</view>
			</view> -->
			<view class="row-end">
				<view class="left_lab">商品小计</view>
				<view class="newPrice" style="color: #C02C38;">￥{{order.need_pay}}</view>
			</view>
		</view>
		...
		<view class="item">
			...
			<view style="padding: 20rpx 30rpx;font-size: 28rpx;color: #333333;line-height: 42rpx;">
		...
		<view class="item">
			...
			<view class="item_row">
				...
				<view class="left_b">{{timestampToTime(order.order_time)}}</view>
			</view>
			<view class="item_row">
			...
			<view class="item_row">
				...
				<view class="left_b">¥{{order.need_pay}}</view>
			</view>
			<view class="btn_row">
		...
		<view class="bottom">
			<view class="btn_b" style="color: #C02C38;border: 1rpx solid #C02C38;" v-if="order.status==2" @click.stop="mask=true">
				我要接单
			</view>
			<!-- <view class="btn_b" style="color: #C02C38;border: 1rpx solid #C02C38;" v-if="order.state==2" @click="mask=true">
				开始配送
			</view> -->
			<view class="btn_b" style="color: #C02C38;border: 1rpx solid #C02C38;" v-if="order.status==3&&order.is_delivery!=1" @click.stop="confirmbtn()">
				{{order.code!='daofu'?'确认送达':'确认送达并收款'}}
			</view>
		...
		methods: {
			nonefb(){
				...
				if(self.order.code!='daofu'){
					Api.getRequestData(self.APIS + '/wxapp/EleStore/startDelivery?orderid=' + self.order.id + '&userid='+self.userid+'&code='+1).then(
						res => {
							console.log(res);
			...
			phoneclick() {
				...
				uni.showModal({
					title: '电话',
					content: '电话号码' + self.order.UserAddr.mobile,
					confirmText: '呼叫',
					success: function(res) {
						if (res.confirm) {
							console.log('用户点击确定');
							uni.makePhoneCall({
								phoneNumber: self.order.UserAddr.mobile //仅为示例
							});
			...
			confirmbtn(item){
				let self = this
				Api.getRequestData(self.APIS + '/wxapp/EleStore/confirmReservationOrder?userid=' + self.userid + '&order_id='+self.order.id).then(
					res => {
						console.log(res);
						if (res.data.message == '确认成功') {
							let pages = getCurrentPages();
							let prePage = pages[pages.length - 2];
							let data = prePage.$vm ? prePage.$vm : prePage
							self.showpop = true
			...
			Receiving() {
				...
				Api.getRequestData(self.APIS + '/wxapp/EleStore/startDelivery?orderid=' + self.order.id + '&userid='+self.userid+'&code='+2).then(
					res => {
						console.log(res);
						if (res.data.message == '修改成功') {
							let pages = getCurrentPages();
			...
			getdata() {
				let self = this
				Api.getRequestData(self.APIS + '/wxapp/user/getAppointmentDetail?order_id=' + self.id).then(
					res => {
						console.log(res);
						self.order = res.data
						let state = Number(self.order.status)
